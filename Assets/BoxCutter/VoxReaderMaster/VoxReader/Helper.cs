        ...
        public static IEnumerable<IModel> ExtractModels(IChunk mainChunk, IPalette palette)
        {
            ...
            foreach (var keyValuePair in transformNodesThatHaveAShapeNode)
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    var voxels = voxelChunks[id].Voxels
                        .Where(v => v.ColorIndex > 0 && v.ColorIndex - 1 < palette.RawColors.Length) // Filter invalid indices
                        .Select(voxel =>
                        {
                            return new Voxel(
                                voxel.Position,
                                ApplyTransformationToVoxel(voxel.Position, globalPos, globalRotation, localSize),
                                palette.RawColors[voxel.ColorIndex - 1], 
                                inverseIndexMap[voxel.ColorIndex - 1]);
                        }).ToArray();
